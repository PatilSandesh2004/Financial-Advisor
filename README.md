...
FastAPI backend + Streamlit ChatGPT-style frontend that explains portfolio moves via a causal chain (macro news → sector → stock → portfolio impact).
...
cp .env.example .env
# edit .env with your GROQ_API_KEY (optional for local fallback)
...
## Project Layout (from blueprint)

- `backend/`: FastAPI app, agent orchestration, intelligence modules, session memory
- `frontend/`: Streamlit UI + SSE client
- `data/`: Mock JSON datasets
- `scripts/`: DB init/seed utilities

## Mock Dataset

This repo includes comprehensive mock data for the challenge.

## Dataset Overview

| File (in `data/`) | Description | Key Data Points |
|------|-------------|-----------------|
